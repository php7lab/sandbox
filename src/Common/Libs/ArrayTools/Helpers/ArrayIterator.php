...
use PhpLab\Sandbox\Common\Helpers\ReflectionHelper;
use PhpLab\Sandbox\Common\Libs\Scenario\Collections\ScenarioCollection;
use PhpLab\Sandbox\Common\Libs\Scenario\helpers\ScenarioHelper;
    ...
    protected function runFilters(Query $query = null, $filters = [self::FILTER_SORT, self::FILTER_WHERE, self::FILTER_OFFSET])
    {
        ...
        foreach ($allFilters as $filterName) {
            if (in_array($filterName, $filters)) {
                $filterConfig = [
                    'class' => 'PhpLab\Sandbox\Common\Libs\ArrayTools\Filters\Iterator\\' . ucfirst($filterName),
                    'query' => $query,
                ];
